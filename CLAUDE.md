...
- **No navigation library** — the screen graph is a shallow kiosk tree (Home → one screen per section → a player/viewer; no deep links), so navigation is a plain sealed-class state machine rather than Navigation-Compose. Revisit only if the screen graph grows meaningfully.
...
- **Appendices**: a curated YouTube catalog in **one file, `assets/appendices/catalog.json`** (categories → collections → videos; parsed by `AppendicesCatalog`, guarded by `AppendicesCatalogTest`; re-check every entry with `python3 tools/appendices/verify_catalog.py [--fix]`). Categories are tabs (`TabRow`) over shelves played in-app by `YouTubePlayerScreen` — `android-youtube-player` (IFrame API in a WebView) with YouTube's controls hidden, driven by the same remote keys and `PlayerOverlay` as the film player; resume positions are saved under `yt_<id>` without becoming the Home banner's last-watched title (`saveProgress(markLastWatched = false)`). Videos under an `Appendices`/`Extras`/`Bonus`/… folder on the drive are extras, never matched as films. The app needs `INTERNET` for this only.
...
- **Middle-earth in 3D** (`ui/vault/WorldScreen.kt` + `assets/world/`): three.js in a WebView served by `WebViewAssetLoader` (`https://appassets.androidplatform.net/assets/…`, fonts via `/res/font/…`), never focusable — Compose forwards remote keys as `World.key('up'|'down'|'left'|'right'|'ok'|'back'|'ff'|'rw')`, and the page reports `{place, stop, canZoomOut}` through the `Android` JavaScript interface so the native `MapHeader`/`MapFooter` overlays show the place or journey stop. Terrain = the Arda DEM (`tools/world/build_world.py` → `height.png` 16-bit packed in R/G, painted `terrain.jpg`, `places.json` with Atlas place ids); scene source in `tools/world/web/src` (figures and landmarks are procedural, no model files), bundled by `npm run build` (esbuild) into `assets/world/world.js` — commit the built bundle. `?demo`, `?demo=<journeyId>&stop=N` or `?at=<placeId>` run it in a desktop browser (serve `app/src/main` so `/assets/` and `/res/` resolve). Arda's data licence is uncertain → personal use only (`third_party/arda/NOTICE.md`).
- **3D story** (`tools/world/web/src/`): `characters.js` (the cast; a bundled model from `assets/world/models/models.json` when there is one, else a built figure), `creatures.js` (Balrog, Smaug, eagles, fell beasts, trolls, mûmakil), `effects.js` (particle fire/fireworks/mist, glow sprites — **no dynamic PointLights**: changing the light count recompiles every shader), `scenes.js` (`COMPANIES`: who walks each leg; `SCENES`: a looping scene per stop, staged in the travellers' local space facing away from the camera). Test a scene in desktop Chrome with `?demo=<journey>&stop=N&t=secs&freeze` (serve `app/src/main`). The WebView console goes to logcat (tag `World`, incl. an fps line). The Android emulator's WebGL runs at ~3 fps, so judge timing on a real TV.
...
- **Music** (`ui/music/`): `MusicRepository` reads audio under a `Music`/`Soundtrack`/`Scores` folder (`READ_MEDIA_AUDIO`) with tags and embedded covers via `MediaMetadataRetriever`, grouped into albums. `MusicScreen` owns its own ExoPlayer (released on leaving — music doesn't play behind other screens), shows a now-playing card, and takes the remote's media keys anywhere on the screen. `ScoreThemes` are Howard Shore's leitmotifs with "listen for" notes, keywords that match drive tracks, and an official "Topic" recording on YouTube (checked embeddable) played in `YouTubePlayerScreen` when the drive has none. Card art without a picture uses `GlyphPlaceholder` (section tint + large Aniron glyph), shared with Reading.
...
- **USB access**: direct file reads with the video-read permission (`READ_MEDIA_VIDEO`, or `READ_EXTERNAL_STORAGE` on API ≤32; `READ_MEDIA_IMAGES` too, asked on entering the Vault or Reading, and `READ_MEDIA_AUDIO` on entering Music), not the SAF picker — Android TV builds often ship without a document-picker app (the stock TV emulator has none). By default `StorageRepository` looks for a `LOTR` folder at the root of the first removable volume (the pendrive) and scans it plus subfolders (films are typically one folder per film); a custom folder chosen in the in-app `FolderBrowser` overrides it and is persisted. To test the pendrive path on the emulator: `adb shell sm set-virtual-disk true`, then `sm partition <disk> public`.
...
- `com.example.lotr.ui` — `theme/` (Compose theme/colors), one package per screen (`home/`, `library/`, `appendices/`, `vault/`, `reading/`, `music/`, `player/`), plus shared `components/`. No direct file/URI/DataStore access here — goes through `data` repositories.
