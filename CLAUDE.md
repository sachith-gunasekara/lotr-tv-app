...
- `com.example.lotr.ui` — `theme/` (Compose theme/colors), one package per screen (`home/`, `library/`, `appendices/`, `vault/`, `player/`), plus shared `components/`. No direct file/URI/DataStore access here — goes through `data` repositories.
